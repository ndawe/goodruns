...
def clipped(grl, startrun = None, startlb = None, endrun = None, endlb = None):
   ...
   grl_copy.clip(startrun = startrun, startlb = startlb, endrun = endrun, endlb = endlb)
   return grl_copy
        ...
        if self.has_run(run):
            for mylbrange in self[run][:]:
                ...
                if lbrange == mylbrange:
                    ...
                    diff = _lbrange_as_set(mylbrange).difference(_lbrange_as_set(lbrange))
                ...
                if run < startrun:
                    ...
                    if startlb is not None:
                        ...
                        for lbrange in lbranges:
                            ...
                            elif startlb >= lbrange[0] and startlb <= lbrange[1]:
                                self.__grl[run][self.__grl[run].index(lbrange)] = (startlb, lbrange[1])
                    ...
                    if endlb is not None:
                        ...
                        for lbrange in lbranges:
                            if lbrange[0] > endlb:
                                ...
                                self[run][self[run].index(lbrange)] = (lbrange[0], endlb)
                        if len(self[run]) == 0:
                            del self[run]
